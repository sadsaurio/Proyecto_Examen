...
void creararchivo(pregunta *&inicio, pregunta *&actual ){
    ...
    while(getline(arch,linea)){
        if(linea.find(":p;")==0){
            ...
            nuevo->correcta=stoi(linea.substr(3)); // stoi convertir en char en int
        }
        else if(isdigit(linea[0])){  // verificar si todos los carecteres de char son digitos numericos
            nuevo->puntos=stof(linea); // en float
            nuevo->sig=NULL;
            nuevo->ant=actual;
            if(inicio==NULL){
                inicio=nuevo;
...
void generarExam(pregunta *&actual){
    int opc;
    do{
        system("cls"); 
        cout<<"Pregunta:\n";
        ...
        cout<<"Puntos: "<<actual->puntos<<endl;
        cout<<"1. Siguiente\n";
        cout<<"2. Anterior\n";
        cout<<"3. Editar\n";
        cout<<"4. Salir\n";
        ...
        cin>>opc;
        switch(opc){
            case 1: if(actual->sig !=NULL){
                       actual=actual->sig;
                    } 
                    break;
            case 2: if(actual->ant!=NULL){
                       actual=actual->ant;
                    }
                    break;
            case 3: cout<<"Nueva Pregunta: ";
                    cin.ignore();
                    getline(cin,actual->cuestion);
                    break;
